...
                // The rest of your existing message formatting logic goes here...
                let replyMessageParts = [];
                // [LOGIC D'AFFICHAGE DU STATUT DE COMMANDE]
                // ... votre code de formatage de message existant
                
                // Example of how to add a line for Numéro - Client
...
            
            if (lowerCaseMessage.includes("produit") || lowerCaseMessage.includes("disponible") || lowerCaseMessage.includes("liste")) {
                console.log('[INTENT] User is asking for products. Fetching from WooCommerce...');
...
${wooProducts.map(p => `
    ...
    URL: ${p.permalink}
`).join('\n---\n')}
Réponds à la question du client en utilisant ces informations. Indique clairement que les prix sont en euros. Si aucun produit ne correspond, dis-le au client.
`;
                } else {
                    console.log('[WOOCOMMERCE] No products found for the query.');
                    productContext = "Aucun produit n'a été trouvé pour cette requête.";
                }
            }
